...
**Regex safety:** Every template, category, and merchant-filter match runs isolated in a bounded worker pool (issue #61) with a per-match timeout (default 100 ms, clamped to 10-1000 ms via `BANK_REGEX_TIMEOUT_MS`). A timed-out or invalid pattern fails the entire tool call — financial results are never silently partial, so a suspiciously small or incomplete result set is surfaced as an error rather than returned as if complete.
